...
function splitFrontmatter(raw) {
  ...
  let fm = {};
  try {
    fm = (0, import_yaml.parse)(fmText) ?? {};
  } catch (e) {
    throw new BriefError(`invalid YAML frontmatter: ${e.message}`);
  }
  return { fm, body: rest };
}
...
function validateBrief(input, requireVerify = true) {
  const missing = [];
...
function fromObject(obj, fallbackId, requireVerify = true) {
  return validateBrief(
    {
      id: typeof obj.id === "string" ? obj.id : fallbackId,
      objective: typeof obj.objective === "string" ? obj.objective : void 0,
      acceptance: typeof obj.acceptance === "string" ? obj.acceptance : typeof obj.acceptance_criteria === "string" ? obj.acceptance_criteria : void 0,
      ...
      paths: typeof obj.paths === "string" ? obj.paths : void 0,
      constraints: typeof obj.constraints === "string" ? obj.constraints : void 0,
      outOfScope: typeof obj.out_of_scope === "string" ? obj.out_of_scope : typeof obj.outOfScope === "string" ? obj.outOfScope : void 0
    },
    requireVerify
  );
}
function loadBriefs(inputPath, requireVerify = true) {
  if (!(0, import_node_fs.existsSync)(inputPath)) throw new BriefError(`input not found: ${inputPath}`);
  const st = (0, import_node_fs.statSync)(inputPath);
  if (st.isDirectory()) {
    const files = (0, import_node_fs.readdirSync)(inputPath).filter((f) => f.endsWith(".md")).sort();
    if (files.length === 0) throw new BriefError(`no .md briefs in directory: ${inputPath}`);
    return files.map(
      (f, i) => parseBrief(
        (0, import_node_fs.readFileSync)((0, import_node_path.join)(inputPath, f), "utf8"),
        slug((0, import_node_path.basename)(f, ".md")) || `task-${i + 1}`,
        requireVerify
      )
    );
  }
  const ext = (0, import_node_path.extname)(inputPath).toLowerCase();
  const raw = (0, import_node_fs.readFileSync)(inputPath, "utf8");
  if (ext === ".json" || ext === ".yaml" || ext === ".yml") {
    const data = ext === ".json" ? JSON.parse(raw) : (0, import_yaml.parse)(raw);
    ...
    return arr.map((o, i) => fromObject(o, `task-${i + 1}`, requireVerify));
  }
  const chunks = raw.replace(/\r\n/g, "\n").split(/\n={3,}\s*\n/);
  if (chunks.length === 1) {
    return [parseBrief(raw, slug((0, import_node_path.basename)(inputPath, ".md")) || "task-1", requireVerify)];
  }
  return chunks.filter((c) => c.trim()).map((c, i) => parseBrief(c, `task-${i + 1}`, requireVerify));
}
function dedupeIds(briefs) {
...
var SENTINEL = "pidelegate-report";
function renderPrompt(brief, opts = {}) {
  const parts = [];
  parts.push(
    opts.readOnly ? "You are an autonomous investigation agent invoked by `pidelegate`. Do NOT modify, create, or delete any files \u2014 read, search, and analyze only, then report findings. You have no prior conversation context \u2014 everything you need is in this brief." : "You are an autonomous implementation agent invoked by `pidelegate`. Implement the task below completely, editing files in the current repository. You have no prior conversation context \u2014 everything you need is in this brief. Work until the acceptance criteria are met; run relevant checks yourself as you go."
  );
  parts.push(`
## Objective
${brief.objective}`);
...
When you are done, output as your final message a fenced code block tagged \`${SENTINEL}\` containing JSON with this shape:
\`\`\`` + SENTINEL + "\n" + JSON.stringify(
      {
...
function renderFixPrompt(brief, verifyTail) {
  return `The verification step failed. Fix the issues and ensure all acceptance criteria are met.

## Failing verification output (tail)
${verifyTail}

Re-read the relevant files, make the necessary changes, then finish with an updated \`${SENTINEL}\` report block as before.`;
}
...
function preflight() {
  const bin = piBin();
  try {
    const version = (0, import_node_child_process.execFileSync)(bin, ["--version"], {
      encoding: "utf8",
    ...
    return {
      ok: false,
      bin,
      error: `pi binary "${bin}" not found or not runnable (${e.message}). Install pi and ensure it is on PATH, or set PIDELEGATE_PI_BIN.`
    };
  }
}
...
function runPi(a) {
  (0, import_node_fs2.mkdirSync)((0, import_node_path2.dirname)(a.eventsFile), { recursive: true });
  const out = (0, import_node_fs2.createWriteStream)(a.eventsFile, { flags: "a" });
  return new Promise((resolve2, reject) => {
    const child = (0, import_node_child_process.spawn)(piBin(), buildArgs(a), {
      cwd: a.cwd,
    ...
    child.on("error", (err) => {
      ...
      if (err.name === "AbortError") {
        resolve2({ exitCode: null, signalled: true });
      } else {
        reject(err);
      }
    });
    child.on("close", (code, sig) => {
      out.end();
      resolve2({ exitCode: code, signalled: sig !== null });
    });
  });
}

// src/delegate/verify.ts
var import_node_child_process2 = require("child_process");
var TAIL_BYTES = 2e3;
function tail(s) {
  return s.length > TAIL_BYTES ? s.slice(s.length - TAIL_BYTES) : s;
}
function runOne(cmd, cwd, signal) {
  return new Promise((resolve2) => {
    const child = (0, import_node_child_process2.spawn)(cmd, {
      cwd,
      shell: true,
      stdio: ["ignore", "pipe", "pipe"],
      signal
    ...
    child.on("error", (err) => {
      resolve2({ cmd, exitCode: 127, outputTail: tail(out + `
[spawn error] ${err.message}`) });
    });
    child.on("close", (code) => {
      resolve2({ cmd, exitCode: code ?? -1, outputTail: tail(out).trim() });
    });
  });
}
async function runVerify(cwd, cmds, signal) {
  const results = [];
...
function failingTail(v) {
  return v.results.filter((r) => r.exitCode !== 0).map((r) => `$ ${r.cmd}
(exit ${r.exitCode})
${r.outputTail}`).join("\n\n").slice(-TAIL_BYTES);
}
...
function readEvents(path) {
  ...
  return parseEventLines((0, import_node_fs3.readFileSync)(path, "utf8"));
}
...
function rootDir() {
  return process.env.PIDELEGATE_HOME ?? (0, import_node_path3.join)((0, import_node_os.homedir)(), ".pidelegate");
}
function jobsDir() {
  return (0, import_node_path3.join)(rootDir(), "jobs");
}
function jobDir(id) {
  return (0, import_node_path3.join)(jobsDir(), id);
}
function taskDir(id, taskId) {
  return (0, import_node_path3.join)(jobDir(id), taskId);
}
function sessionsDir(id) {
  return (0, import_node_path3.join)(jobDir(id), "sessions");
}
function sessionPath(id, taskId) {
  return (0, import_node_path3.join)(sessionsDir(id), `${taskId}.jsonl`);
}
function eventsPath(id, taskId) {
  return (0, import_node_path3.join)(taskDir(id, taskId), "events.jsonl");
}
function taskResultPath(id, taskId) {
  return (0, import_node_path3.join)(taskDir(id, taskId), "result.json");
}
function metaPath(id) {
  return (0, import_node_path3.join)(jobDir(id), "meta.json");
}
function resultPath(id) {
  return (0, import_node_path3.join)(jobDir(id), "result.json");
}
function supervisorLogPath(id) {
  return (0, import_node_path3.join)(jobDir(id), "supervisor.log");
}
function abortFlagPath(id) {
  return (0, import_node_path3.join)(jobDir(id), "abort.flag");
}
function newJobId() {
  return (0, import_node_crypto.randomUUID)().slice(0, 8);
}
function ensureJobDirs(id, taskIds) {
  (0, import_node_fs4.mkdirSync)((0, import_node_path3.join)(jobDir(id), "sessions"), { recursive: true });
  for (const t of taskIds) (0, import_node_fs4.mkdirSync)(taskDir(id, t), { recursive: true });
}
function writeMeta(meta) {
  meta.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
  (0, import_node_fs4.mkdirSync)(jobDir(meta.id), { recursive: true });
  atomicWrite(metaPath(meta.id), JSON.stringify(meta, null, 2));
}
function readMeta(id) {
  const p = metaPath(id);
  if (!(0, import_node_fs4.existsSync)(p)) throw new Error(`unknown job: ${id}`);
  return JSON.parse((0, import_node_fs4.readFileSync)(p, "utf8"));
}
function briefsPath(id) {
  return (0, import_node_path3.join)(jobDir(id), "briefs.json");
}
function writeBriefs(id, briefs) {
  (0, import_node_fs4.mkdirSync)(jobDir(id), { recursive: true });
  atomicWrite(briefsPath(id), JSON.stringify(briefs, null, 2));
}
function readBriefs(id) {
  return JSON.parse((0, import_node_fs4.readFileSync)(briefsPath(id), "utf8"));
}
function writeResult(id, rollup2) {
  atomicWrite(resultPath(id), JSON.stringify(rollup2, null, 2));
}
function readResult(id) {
  const p = resultPath(id);
  return (0, import_node_fs4.existsSync)(p) ? JSON.parse((0, import_node_fs4.readFileSync)(p, "utf8")) : null;
}
function writeTaskResult(id, taskId, data) {
  atomicWrite(taskResultPath(id, taskId), JSON.stringify(data, null, 2));
}
function readTaskResult(id, taskId) {
  const p = taskResultPath(id, taskId);
  return (0, import_node_fs4.existsSync)(p) ? JSON.parse((0, import_node_fs4.readFileSync)(p, "utf8")) : null;
}
function clearAbort(id) {
  if ((0, import_node_fs4.existsSync)(abortFlagPath(id))) (0, import_node_fs4.rmSync)(abortFlagPath(id));
}
function jobExists(id) {
  return (0, import_node_fs4.existsSync)(metaPath(id));
}
function listJobIds() {
  const dir = jobsDir();
  if (!(0, import_node_fs4.existsSync)(dir)) return [];
  return (0, import_node_fs4.readdirSync)(dir).filter((name) => (0, import_node_fs4.existsSync)(metaPath(name))).sort((a, b) => mtime(metaPath(b)) - mtime(metaPath(a)));
}
...
function requestAbort(id) {
  (0, import_node_fs4.writeFileSync)(abortFlagPath(id), (/* @__PURE__ */ new Date()).toISOString());
}
function abortRequested(id) {
  return (0, import_node_fs4.existsSync)(abortFlagPath(id));
}
function mtime(p) {
  try {
    return (0, import_node_fs4.statSync)(p).mtimeMs;
  } catch {
    return 0;
  }
}
...
// src/delegate/report.ts
var SUMMARY_CAP = 800;
function parseSelfReport(finalText) {
  ...
  if (m) {
    try {
      ...
      return {
        ...
        acceptanceMet: typeof obj.acceptance_met === "boolean" ? obj.acceptance_met : null,
  ...
  return { summary: text || void 0, acceptanceMet: null, blockers: [] };
}
function cleanBlockers(value) {
  if (!Array.isArray(value)) return [];
  return value.filter((b) => typeof b === "string").map((b) => b.trim()).filter((b) => b && !/^<.*>$/.test(b) && b.toLowerCase() !== "none" && b.toLowerCase() !== "empty");
}
function truncate(s, cap = SUMMARY_CAP) {
  return s.length > cap ? s.slice(0, cap - 1) + "\u2026" : s;
}
function buildTaskResult(args) {
  ...
  const summary = report.summary?.trim() || (status === "success" ? verify.ran ? "Completed; verification passed." : "Completed." : "No summary reported by the delegate.");
  return {
    taskId: brief.id,
    status,
    ...
    acceptance: {
      ...
      ...report.acceptanceMet === false && verify.passed ? { notes: "delegate self-reported acceptance not met despite green verify" } : {}
    },
...
function rollup(jobId, cwd, status, tasks, piVersion = null) {
...
// src/delegate/gitstatus.ts
var import_node_child_process3 = require("child_process");
function porcelain(cwd) {
  try {
    const out = (0, import_node_child_process3.execFileSync)("git", ["status", "--porcelain"], {
      cwd,
      encoding: "utf8",
      stdio: ["ignore", "pipe", "ignore"]
    });
...
function mergeChanges(gitChanges, toolChanges) {
...
// src/delegate/worktree.ts
var import_node_child_process4 = require("child_process");
var import_node_crypto2 = require("crypto");
var import_node_os2 = require("os");
var import_node_path4 = require("path");
function git(cwd, args) {
  return (0, import_node_child_process4.execFileSync)("git", args, { cwd, encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }).trim();
}
...
function createWorktree(cwd, jobId, taskId) {
  const branch = `pidelegate/${jobId}/${taskId}`;
  const dir = (0, import_node_path4.join)((0, import_node_os2.tmpdir)(), `pidg-wt-${jobId}-${taskId}-${(0, import_node_crypto2.randomBytes)(4).toString("hex")}`);
  git(cwd, ["worktree", "add", "-b", branch, dir, "HEAD"]);
  return { dir, branch };
}
function commitAll(wtDir, message) {
  git(wtDir, ["add", "-A"]);
...
function mergeBranch(cwd, branch) {
  try {
    ...
    return { ok: false, conflict: true, message: e.message.split("\n")[0] };
  }
}
function removeWorktree(cwd, wt) {
  try {
...
// src/delegate/supervisor.ts
function log(...args) {
  console.log(`[${(/* @__PURE__ */ new Date()).toISOString()}]`, ...args);
}
async function runTask(meta, brief, controller, execCwd) {
  const jobId = meta.id;
  ...
  let fixIterations = 0;
  let verify = hasVerify ? await runVerify(execCwd, brief.verify, controller.signal) : { passed: false, ran: false, results: [] };
...
async function executeTask(meta, brief, controller, execCwd) {
  try {
    return await runTask(meta, brief, controller, execCwd);
  } catch (err) {
    log(`task ${brief.id}: error ${err.message}`);
    const result = {
      ...abortedResult(meta.id, brief),
      status: "failed",
      summary: `Delegate errored: ${err.message}`
    };
    writeTaskResult(meta.id, brief.id, result);
    return result;
  }
}
async function runSequential(meta, briefs, controller, results) {
  for (const brief of briefs) {
    const existing = meta.tasks.find((t) => t.id === brief.id);
    if (existing?.status === "success") {
      const prior = readTaskResult(meta.id, brief.id);
      if (prior) {
...
async function runParallel(meta, briefs, controller, results) {
  ...
  const worker = async () => {
    for (; ; ) {
      ...
      try {
        ...
        const committed = !controller.signal.aborted && result.status !== "aborted" && commitAll(wt.dir, `pidelegate: ${b.id}`);
        outcomes.push({ brief: b, result, committed, wt });
      } catch (err) {
        log(`parallel: ${b.id} setup/run error ${err.message}`);
        outcomes.push({
          brief: b,
          result: { ...abortedResult(meta.id, b), status: "failed", summary: `Delegate errored: ${err.message}` },
          committed: false,
          wt
        });
      }
...
async function runSupervisor(jobId) {
...
function setTaskStatus(meta, taskId, status) {
  const t = meta.tasks.find((x) => x.id === taskId);
  if (t) t.status = status;
}
function persistAborted(meta, brief) {
...
function abortedResult(jobId, brief) {
  return {
    taskId: brief.id,
    status: "aborted",
...
// src/delegate/commands.ts
var TERMINAL = ["done", "aborted", "error"];
var isTerminal = (s) => TERMINAL.includes(s);
function progressOf(meta) {
  const byStatus = {};
...
function reconcile(id) {
  ...
  const orphaned = (meta.status === "running" || meta.status === "queued") && meta.supervisorPid != null && !isProcessAlive(meta.supervisorPid);
  if (!orphaned) return meta;
  meta.status = "error";
  meta.error = meta.error ?? "supervisor process is no longer running (crashed or was killed)";
  meta.finishedAt = meta.finishedAt ?? (/* @__PURE__ */ new Date()).toISOString();
  writeMeta(meta);
  if (!readResult(id)) {
    const tasks = meta.tasks.map((t) => readTaskResult(id, t.id)).filter((r) => r != null);
    writeResult(id, rollup(id, meta.cwd, "error", tasks, meta.piVersion));
  }
  return meta;
}
async function cmdRun(argv, entry) {
  ...
  let briefs;
  try {
    briefs = dedupeIds(loadBriefs((0, import_node_path5.resolve)(input), !readOnly));
  } catch (e) {
    return fail(e.message);
  }
  const cwd = (0, import_node_path5.resolve)(values.cwd ?? process.cwd());
  const tools = values.tools ?? (readOnly ? "read,grep,find,ls" : void 0);
  const options = {
    ...
    timeoutMs: parseDuration(values.timeout, 30 * 6e4),
  ...
  if (values.wait) {
    await waitLoop(id, parseDuration(values.budget, 8 * 6e4));
    return;
  }
  printJson({
    ok: true,
...
function spawnSupervisor(id, entry, cwd) {
  const logFd = (0, import_node_fs5.openSync)(supervisorLogPath(id), "a");
  const child = (0, import_node_child_process5.spawn)(process.execPath, [entry, "delegate", "__supervise", id], {
    cwd,
    detached: true,
...
async function cmdWait(argv) {
  ...
  await waitLoop(id, parseDuration(values.budget, 8 * 6e4), parseDuration(values.interval, 1e3));
}
async function waitLoop(id, budgetMs, intervalMs = 1e3) {
...
function cmdPlan(argv) {
  ...
  void from;
  fail(
    "delegate plan is not implemented yet. For now, the host should write the task brief(s) directly and call `delegate run`."
  );
}
async function cmdSupervise(argv) {
  const id = argv[0];
  if (!id) {
...
var HELP = `pidelegate \u2014 offload implementation work to the pi coding agent.
...
async function delegateMain(argv, entry) {
  const sub = argv[0];
...
var TOP_HELP = `@zkareemz/skills \u2014 a collection of agent skills.
